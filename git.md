...
mkdir 目录名  #创建目录，目录名称不要有中文
cd learngit # 进入目录
pwd   #显示当前目录。
git init    #初始化仓库 将仓库交给git管理    
...
```bash
git add readme.txt   # 将文件提交到git仓库暂存区
git add --all # 提交当前目录下所有改动文件到暂存区（会记录删除操作）
...
git push -u origin master# 将本地仓库内容推送到远程仓库上
